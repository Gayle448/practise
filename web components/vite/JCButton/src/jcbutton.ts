...
@customElement('jc-button')
export class JCButton extends LitElement {
  @property()
  content = ''
  ...
  render() {
    ...
    return html`
        <button id="element" class=${classMap(this.myClass)} @click="${this.clickHandler}" style=${styleMap(this.myStyle)}>
            <img class="loadingStyle disable" ?hidden=${!this.loading} src="src/assets/images/loading.png" />
            <img class=${this.icon ? "iconStyle" : ''} src="src/assets/images/${this.icon || nothing}.png" />
            <span>
                ${this.textContent}
            </span>
            <slot name="icon"></slot>
